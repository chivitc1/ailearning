...
def find_similar_users(dataset, user, num_users):
    """to find the users in the dataset that are similar to the given user."""

    # If the user is not in the dataset, raise an error
...
if __name__ == '__main__':
    ...
    user = args.user
    ratings_file = 'knn/ratings.json'
    with open(ratings_file, 'r') as f:
        data = json.loads(f.read())

    print('\nUsers similar to ' + user + ':\n')
    similar_users = find_similar_users(data, user, num_users=3)
